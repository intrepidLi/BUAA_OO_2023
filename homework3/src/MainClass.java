...
public class MainClass {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < funcnt; i++) {
            ...
            String newfun = fun.replaceAll(" ", "").replaceAll("\t", "");
            String fun1 = Lexer.deleteMulAddProcess(Lexer.preProcess(newfun));
            funcs.proFun(fun1);
        }
        String input = scanner.nextLine();
        String newInput = input.replaceAll(" ","").replaceAll("\t", "");
        String newnewInput = funcs.subFun(newInput);
        newnewInput = Lexer.deleteMulAddProcess(Lexer.preProcess(newnewInput));
    ...
    public static Item simplify(Expr expr) {
        String[] expressions = expr.toString().split(" ");
        Stack<Item> stack = new Stack<Item>();
        int flag = 0;
        for (String expression : expressions) {
            if (flag == 0) {
                stack.push(new Item(expression));
                flag = 1;
                continue;
            }
            ...
            else if (expression.equals("dx") | expression.equals("dy") | expression.equals("dz")) {
                derive(stack, expression);
            }
            else {
    ...
    public static String simPlerr(String str) {
        StringBuilder sb = new StringBuilder();
        //String sourfunc = "\\(1\\*(?<cont>[\\w*]+?)\\*1\\)";
        //Pattern pattern = Pattern.compile(sourfunc);
    ...
    private static void sinCos(Stack<Item> stack, String s) {
        if (s.equals("cos")) {
    ...
    private static void derive(Stack<Item> stack, String s) {
        Item item1 = stack.get(stack.size() - 1);
        Item item = item1.diff(s);
        stack.pop();
        stack.push(item);
    }
    ...
    /*public static String getDiff(String diffname, String str) {
        Lexer lexer = new Lexer(str);
        Parser parser = new Parser(lexer);
        Expr expr = parser.parseExpr();
        Item newstr = simplify(expr);
    }*/
}
